...
texreg <- function(l, file = NULL, single.row = FALSE, 
    ...
    dcolumn = FALSE, longtable = FALSE, sideways = FALSE, use.packages = TRUE, table = TRUE, 
  ...
  # check longtable vs. table
  if (table == TRUE && longtable == TRUE) {
    table <- FALSE
    msg <- paste("The longtable package and the table argument cannot be used at", 
                 "the same time. Switching off table.")
    warning(msg)
  }
  
   # check longtable vs. sidewyas
  if (longtable == TRUE && sideways == TRUE) {
    sideways <- FALSE
    msg <- paste("The longtable package and sideways environment cannot be used at",
                 "the same time. You may want to use the pdflscape package. Switching off sideways.")
    warning(msg)
  }
  ...
  if (longtable == TRUE && !(float.pos %in% c("", "l", "c", "r"))) {
    float.pos <- ""
    msg <- paste("When the longtable environment is used, the float.pos argument",
                 "can only take one of the 'l', 'c', 'r', or '' (empty) values.",
                 "Setting float.pos value to '' (empty).")
    warning(msg)
  }
  ...
  if (use.packages == TRUE) {
    ...
    if (dcolumn == TRUE || booktabs == TRUE || sideways == TRUE || longtable == TRUE) {
      string <- paste0(string, "\n")
    }
  }
  if (table == TRUE) {
  ...
  if (longtable != TRUE) string <- paste0(string, "\n\\begin{tabular}")
  ...
  if (booktabs == TRUE) {
    # caption 
    if (longtable == TRUE && caption.above == TRUE) {
      string <- paste0(string, "}\n", "\\caption{", caption, "}\n", "\\toprule\n")
    } else { 
      string <- paste0(string, "}\n", "\\toprule\n")
    }
  ...
  # footer for longtables
  if (longtable == TRUE) {
    string <- paste0(string, "\\endhead\n")
    if (booktabs == TRUE) {
    ...
    string <- paste0(string, "\\endlastfoot\n")
